import '../BluefinTecsUserBackofficeError.dart';
import '../Result.dart';
...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'BluefinTecsUserBackofficeSDK: ' + opname.toString() + ': ' + errmsg(err);

  BluefinTecsUserBackofficeError sdkerr;
  if (err is BluefinTecsUserBackofficeError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = BluefinTecsUserBackofficeError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
