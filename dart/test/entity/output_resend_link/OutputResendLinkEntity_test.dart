...
void tests() {
  describe('OutputResendLinkEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputResendLink();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_resend_link.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_resend_link01', 'output_resend_link02', 'output_resend_link03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID'];
