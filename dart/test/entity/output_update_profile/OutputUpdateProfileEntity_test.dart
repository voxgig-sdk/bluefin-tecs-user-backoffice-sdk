...
void tests() {
  describe('OutputUpdateProfileEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputUpdateProfile();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputUpdateProfile();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_update_profile.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_update_profile_ref01_ent = client.OutputUpdateProfile();
      dynamic output_update_profile_ref01_data = setup['data']['new']['output_update_profile']['output_update_profile_ref01'];

      output_update_profile_ref01_data = await output_update_profile_ref01_ent.create(output_update_profile_ref01_data);
      ok(null != output_update_profile_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_update_profile01', 'output_update_profile02', 'output_update_profile03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_UPDATE_PROFILE_ENTID'];
