...
void tests() {
  describe('OutputDeactivateUserEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputDeactivateUser();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputDeactivateUser();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_deactivate_user.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_deactivate_user_ref01_ent = client.OutputDeactivateUser();
      dynamic output_deactivate_user_ref01_data = setup['data']['new']['output_deactivate_user']['output_deactivate_user_ref01'];

      output_deactivate_user_ref01_data = await output_deactivate_user_ref01_ent.create(output_deactivate_user_ref01_data);
      ok(null != output_deactivate_user_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_deactivate_user01', 'output_deactivate_user02', 'output_deactivate_user03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_DEACTIVATE_USER_ENTID'];
