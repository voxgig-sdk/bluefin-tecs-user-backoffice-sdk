...
void tests() {
  describe('OutputCreateMandatorEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputCreateMandator();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputCreateMandator();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_create_mandator.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_MANDATOR_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final output_create_mandator_ref01_ent = client.OutputCreateMandator();
      dynamic output_create_mandator_ref01_data = setup['data']['new']['output_create_mandator']['output_create_mandator_ref01'];

      output_create_mandator_ref01_data = await output_create_mandator_ref01_ent.create(output_create_mandator_ref01_data);
      ok(null != output_create_mandator_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_create_mandator01', 'output_create_mandator02', 'output_create_mandator03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_MANDATOR_ENTID'];
