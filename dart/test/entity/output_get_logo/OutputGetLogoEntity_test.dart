...
void tests() {
  describe('OutputGetLogoEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputGetLogo();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['load']) {
        if (maybeSkipControl(t, 'entityOp', 'output_get_logo.' + op, live)) {
          return;
        }
      }
      ...
      final output_get_logo_ref01_data =
          (setup['data']['existing']['output_get_logo'] as Map).values.first;
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_get_logo01', 'output_get_logo02', 'output_get_logo03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_GET_LOGO_ENTID'];
