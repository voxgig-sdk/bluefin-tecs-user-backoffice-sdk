...
void tests() {
  describe('OutputRegisterUserEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputRegisterUser();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputRegisterUser();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_register_user.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_register_user01', 'output_register_user02', 'output_register_user03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_REGISTER_USER_ENTID'];
