...
void tests() {
  describe('OutputActivateUserEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputActivateUser();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputActivateUser();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_activate_user.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_activate_user_ref01_ent = client.OutputActivateUser();
      dynamic output_activate_user_ref01_data = setup['data']['new']['output_activate_user']['output_activate_user_ref01'];

      output_activate_user_ref01_data = await output_activate_user_ref01_ent.create(output_activate_user_ref01_data);
      ok(null != output_activate_user_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_activate_user01', 'output_activate_user02', 'output_activate_user03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_USER_ENTID'];
