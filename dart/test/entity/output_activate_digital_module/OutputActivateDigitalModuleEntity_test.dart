...
void tests() {
  describe('OutputActivateDigitalModuleEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsUserBackofficeSDK.test();
      final ent = testsdk.OutputActivateDigitalModule();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputActivateDigitalModule();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_activate_digital_module.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_activate_digital_module_ref01_ent = client.OutputActivateDigitalModule();
      dynamic output_activate_digital_module_ref01_data = setup['data']['new']['output_activate_digital_module']['output_activate_digital_module_ref01'];

      output_activate_digital_module_ref01_data = await output_activate_digital_module_ref01_ent.create(output_activate_digital_module_ref01_data);
      ok(null != output_activate_digital_module_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_activate_digital_module01', 'output_activate_digital_module02', 'output_activate_digital_module03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_DIGITAL_MODULE_ENTID'];
