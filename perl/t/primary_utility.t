#!perl
# BluefinTecsUserBackoffice SDK primary utility test
#
...
#   live blessed context with BluefinTecsUserBackofficeOmni::livectx before calling a
...
my $_runner = BluefinTecsUserBackofficeOmni::make_runner($TEST_JSON, BluefinTecsUserBackofficeSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinTecsUserBackofficeBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinTecsUserBackofficeBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinTecsUserBackofficeError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinTecsUserBackofficeOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinTecsUserBackofficeOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinTecsUserBackofficeBaseFeature->new;
...
{
  my $hook_client = BluefinTecsUserBackofficeSDK->test(undef, undef);
...
{
  my $init_client = BluefinTecsUserBackofficeSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinTecsUserBackofficeSDK->new({
...
{
  my $blocked_client = BluefinTecsUserBackofficeSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinTecsUserBackofficeOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinTecsUserBackofficeSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinTecsUserBackofficeSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makePoint', sub {
  my ($view) = @_;
  my ($point, $err) = $utility->{make_point}->(BluefinTecsUserBackofficeOmni::livectx($view));
  # The corpus asserts refusals by code (`match: {out: {code: ...}}`), so
  # an error is the RESULT here, flattened to the map the corpus reads.
  return BluefinTecsUserBackofficeOmni::errify($err) if defined $err;
  return $point;
});
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinTecsUserBackofficeOmni::livectx($view);
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinTecsUserBackofficeOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinTecsUserBackofficeOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinTecsUserBackofficeSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinTecsUserBackofficeOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinTecsUserBackofficeOmni::livectx($view));
});
