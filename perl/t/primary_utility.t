...

{
  ...
  my $spec_client = BluefinTecsUserBackofficeSDK->test(undef, $setup_opts);
...
runset('operator.basic', get_spec($PRIMARY, 'operator', 'basic'), sub {
  my ($entry) = @_;
  my $in_val = $entry->{in} || {};
  my $op = BluefinTecsUserBackofficeOperation->new($in_val);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...

{
  ...
  my $auth_client = BluefinTecsUserBackofficeSDK->test(undef, $setup_opts);
