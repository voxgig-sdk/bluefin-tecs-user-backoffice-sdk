...
# operation pipeline. $features is a list of { name =>, options => }.
{
  ...
  sub new {
    ...
    $self->{utility} = BluefinTecsUserBackofficeUtility->new;
    ...
    for my $fspec (@$features) {
      ...
      my $f = BluefinTecsUserBackofficeFeatures::make_feature($name);
  ...
  sub op {
    ...
    my $out = eval {
      ...
      if (Scalar::Util::blessed($ctx->{out}{point})
        && $ctx->{out}{point}->isa('BluefinTecsUserBackofficeError')) {
        die $ctx->{out}{point};
      }
      ...
      $ctx->{response} = Voxgig::Struct::ismap($fetched)
        ? BluefinTecsUserBackofficeResponse->new($fetched) : undef;
    ...
    if (my $err = $@) {
      if (Scalar::Util::blessed($err) && $err->isa('BluefinTecsUserBackofficeError')) {
        $ctx->{ctrl}{err} = $err;
  ...
  sub populate_result {
    ...
    my $result = BluefinTecsUserBackofficeResult->new({});
    $ctx->{result} = $result;
...
if (has_feature('retry')) {
...

  {
    ...
    my ($server, $calls) = recording_server(sub {
      my ($n) = @_;
      return $n < 3
        ? (undef, BluefinTecsUserBackofficeError->new('boom', 'boom'))
