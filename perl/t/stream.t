#!perl
# BluefinTecsUserBackoffice SDK entity stream() test
#
...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinTecsUserBackofficeConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinTecsUserBackofficeHelpers::to_map(
    BluefinTecsUserBackofficeHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinTecsUserBackofficeHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinTecsUserBackofficeHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinTecsUserBackofficeHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinTecsUserBackofficeSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinTecsUserBackofficeSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
