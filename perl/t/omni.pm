...
#    `contextify` answers a TIED plain hash (BluefinTecsUserBackofficeOmniViewTie) whose
...
package BluefinTecsUserBackofficeOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinTecsUserBackofficeOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinTecsUserBackofficeError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinTecsUserBackofficeResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsUserBackofficeOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinTecsUserBackofficeOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinTecsUserBackofficeOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinTecsUserBackofficeOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinTecsUserBackofficeOmniProviderTie;
...
package BluefinTecsUserBackofficeOmniProvider;
...
# The resolver proper.
# --------------------------------------------------------------------------
package BluefinTecsUserBackofficeOmni;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinTecsUserBackofficeOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsUserBackofficeOmniViewTie') ? 1 : 0;
}
...
sub enrich {
  ...
  if (Voxgig::Struct::ismap($ctxmap->{spec}) && !Scalar::Util::blessed($ctxmap->{spec})) {
    $ctx->{spec} = BluefinTecsUserBackofficeSpec->new($ctxmap->{spec});
  }

  my $res_map = $ctxmap->{result};
  if (Voxgig::Struct::ismap($res_map) && !Scalar::Util::blessed($res_map)) {
    $ctx->{result} = BluefinTecsUserBackofficeResult->new($res_map);
    if (Voxgig::Struct::ismap($res_map->{err})
      && defined $res_map->{err}{message} && !ref $res_map->{err}{message}) {
      $ctx->{result}{err} = BluefinTecsUserBackofficeError->new('', $res_map->{err}{message});
    }
  }

  my $resp_map = $ctxmap->{response};
  if (Voxgig::Struct::ismap($resp_map) && !Scalar::Util::blessed($resp_map)) {
    $ctx->{response} = BluefinTecsUserBackofficeResponse->new($resp_map);
    if (BluefinTecsUserBackofficeHelpers::rb_truthy($resp_map->{body})) {
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinTecsUserBackofficeHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinTecsUserBackofficeOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinTecsUserBackofficeOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinTecsUserBackofficeContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinTecsUserBackofficeOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinTecsUserBackofficeOmniProvider';
}
