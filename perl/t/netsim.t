...
{
  my $sdk = BluefinTecsUserBackofficeSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinTecsUserBackofficeSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinTecsUserBackofficeSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinTecsUserBackofficeSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinTecsUserBackofficeSDK->test(undef, undef);
