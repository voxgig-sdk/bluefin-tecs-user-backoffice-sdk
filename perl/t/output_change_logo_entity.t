...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_CHANGE_LOGO_ENTID JSON to run live');
  ...
  $V{output_change_logo_ref01_data} = BluefinTecsUserBackofficeHelpers::to_map($V{output_change_logo_ref01_data_result});
...
sub output_change_logo_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_CHANGE_LOGO_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsUserBackofficeHelpers::to_map($env->{'BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_CHANGE_LOGO_ENTID'});
