...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID JSON to run live');
  ...
  $V{output_resend_link_ref01_data} = BluefinTecsUserBackofficeHelpers::to_map(ref($V{output_resend_link_ref01_data_result}) && $V{output_resend_link_ref01_data_result}->can('data_get') ? $V{output_resend_link_ref01_data_result}->data_get : $V{output_resend_link_ref01_data_result});
...
sub output_resend_link_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsUserBackofficeHelpers::to_map($env->{'BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID'});
