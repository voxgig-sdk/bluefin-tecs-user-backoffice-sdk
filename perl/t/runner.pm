...
package BluefinTecsUserBackofficeTestRunner;
...
sub env_override {
  my ($m) = @_;
  my $live = getenv('BLUEFIN_TECS_USER_BACKOFFICE_TEST_LIVE');
  my $override = getenv('BLUEFIN_TECS_USER_BACKOFFICE_TEST_OVERRIDE');
  ...
  my $explain = getenv('BLUEFIN_TECS_USER_BACKOFFICE_TEST_EXPLAIN');
  $m->{'BLUEFIN_TECS_USER_BACKOFFICE_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
...
sub is_control_skipped {
  ...
  my $skip = BluefinTecsUserBackofficeHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_client_options {
  my $ctrl = load_test_control();
  my $opts = BluefinTecsUserBackofficeHelpers::gpath($ctrl, 'test.client.options');
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinTecsUserBackofficeHelpers::gpath($ctrl, 'test.live.delayMs');
