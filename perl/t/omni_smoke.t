...
sub smoke_pack {
  my $runner = BluefinTecsUserBackofficeOmni::make_runner($SPEC, BluefinTecsUserBackofficeSDK->test(undef, undef));
  return $runner->('smoke');
}
...
# A wrong result fails, with an OmniError naming the mismatch.
{
  ...
  my $err = $@;
  ok(BluefinTecsUserBackofficeOmni::is_omni_error($err), 'the failure is an OmniError');
...
# must fail.
{
  ...
  my $err = $@;
  ok(BluefinTecsUserBackofficeOmni::is_omni_error($err), 'the missing-error failure is an OmniError');
