# BluefinTecsUserBackoffice SDK OutputListOfMandator entity
...
package OutputListOfMandatorEntity;
...
sub make {
  ...
  return OutputListOfMandatorEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputListOfMandator data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputListOfMandator fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputListOfMandator.
#
# reqdata: body data hashref (OutputListOfMandatorCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfMandator data (hashref); dies
