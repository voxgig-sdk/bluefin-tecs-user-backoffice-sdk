# BluefinTecsUserBackoffice SDK OutputRegisterUser entity
...
package OutputRegisterUserEntity;
...
sub make {
  ...
  return OutputRegisterUserEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputRegisterUser data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputRegisterUser fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputRegisterUser.
#
# reqdata: body data hashref (OutputRegisterUserCreateData shape). ctrl: optional
# per-call control. Returns the created OutputRegisterUser data (hashref); dies
