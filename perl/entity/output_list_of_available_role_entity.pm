# BluefinTecsUserBackoffice SDK OutputListOfAvailableRole entity
...
package OutputListOfAvailableRoleEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_list_of_available_role',
    _client => $client,
...
sub make {
  ...
  return OutputListOfAvailableRoleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputListOfAvailableRole data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputListOfAvailableRole fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputListOfAvailableRole.
#
# reqdata: body data hashref (OutputListOfAvailableRoleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfAvailableRole data (hashref); dies
