# BluefinTecsUserBackoffice SDK OutputListOfRoleGroup entity
...
package OutputListOfRoleGroupEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_list_of_role_group',
    _client => $client,
...
sub make {
  ...
  return OutputListOfRoleGroupEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputListOfRoleGroup data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputListOfRoleGroup fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputListOfRoleGroup.
#
# reqdata: body data hashref (OutputListOfRoleGroupCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfRoleGroup data (hashref); dies
