# BluefinTecsUserBackoffice SDK OutputListOfModule entity
...
package OutputListOfModuleEntity;
...
sub make {
  ...
  return OutputListOfModuleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputListOfModule data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputListOfModule fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputListOfModule.
#
# reqdata: body data hashref (OutputListOfModuleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfModule data (hashref); dies
