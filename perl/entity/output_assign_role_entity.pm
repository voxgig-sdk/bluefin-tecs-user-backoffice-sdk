# BluefinTecsUserBackoffice SDK OutputAssignRole entity
...
package OutputAssignRoleEntity;
...
sub make {
  ...
  return OutputAssignRoleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputAssignRole data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputAssignRole fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputAssignRole.
#
# reqdata: body data hashref (OutputAssignRoleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputAssignRole data (hashref); dies
