# BluefinTecsUserBackoffice SDK OutputCreateMandator entity
...
package OutputCreateMandatorEntity;
...
sub make {
  ...
  return OutputCreateMandatorEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputCreateMandator data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputCreateMandator fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputCreateMandator.
#
# reqdata: body data hashref (OutputCreateMandatorCreateData shape). ctrl: optional
# per-call control. Returns the created OutputCreateMandator data (hashref); dies
