# BluefinTecsUserBackoffice SDK OutputActivateUser entity
...
package OutputActivateUserEntity;
...
sub make {
  ...
  return OutputActivateUserEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputActivateUser data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputActivateUser fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputActivateUser.
#
# reqdata: body data hashref (OutputActivateUserCreateData shape). ctrl: optional
# per-call control. Returns the created OutputActivateUser data (hashref); dies
