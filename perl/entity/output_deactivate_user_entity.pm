# BluefinTecsUserBackoffice SDK OutputDeactivateUser entity
...
package OutputDeactivateUserEntity;
...
sub make {
  ...
  return OutputDeactivateUserEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputDeactivateUser data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputDeactivateUser fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputDeactivateUser.
#
# reqdata: body data hashref (OutputDeactivateUserCreateData shape). ctrl: optional
# per-call control. Returns the created OutputDeactivateUser data (hashref); dies
