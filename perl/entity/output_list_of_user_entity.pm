# BluefinTecsUserBackoffice SDK OutputListOfUser entity
...
package OutputListOfUserEntity;
...
sub make {
  ...
  return OutputListOfUserEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputListOfUser data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputListOfUser fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputListOfUser.
#
# reqdata: body data hashref (OutputListOfUserCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfUser data (hashref); dies
