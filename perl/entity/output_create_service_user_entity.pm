# BluefinTecsUserBackoffice SDK OutputCreateServiceUser entity
...
package OutputCreateServiceUserEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_create_service_user',
    _client => $client,
...
sub make {
  ...
  return OutputCreateServiceUserEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputCreateServiceUser data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputCreateServiceUser fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputCreateServiceUser.
#
# reqdata: body data hashref (OutputCreateServiceUserCreateData shape). ctrl: optional
# per-call control. Returns the created OutputCreateServiceUser data (hashref); dies
