# BluefinTecsUserBackoffice SDK OutputResetPassword entity
...
package OutputResetPasswordEntity;
...
sub make {
  ...
  return OutputResetPasswordEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputResetPassword data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputResetPassword fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputResetPassword.
#
# reqdata: body data hashref (OutputResetPasswordCreateData shape). ctrl: optional
# per-call control. Returns the created OutputResetPassword data (hashref); dies
