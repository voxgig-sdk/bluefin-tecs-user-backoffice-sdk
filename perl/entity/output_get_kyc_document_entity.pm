# BluefinTecsUserBackoffice SDK OutputGetKycDocument entity
...
package OutputGetKycDocumentEntity;
...
sub make {
  ...
  return OutputGetKycDocumentEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputGetKycDocument data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputGetKycDocument fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputGetKycDocument.
#
# reqdata: body data hashref (OutputGetKycDocumentCreateData shape). ctrl: optional
# per-call control. Returns the created OutputGetKycDocument data (hashref); dies
