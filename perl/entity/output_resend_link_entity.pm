# BluefinTecsUserBackoffice SDK OutputResendLink entity
...
package OutputResendLinkEntity;
...
sub make {
  ...
  return OutputResendLinkEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputResendLink data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputResendLink fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputResendLink.
#
# reqdata: body data hashref (OutputResendLinkCreateData shape). ctrl: optional
# per-call control. Returns the created OutputResendLink data (hashref); dies
