...
package OutputListOfTransactionsHistoryEntity;
...
sub new {
  ...
  elsif (BluefinTecsUserBackofficeHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
  ...
  my $self = bless {
    _name => 'output_list_of_transactions_history',
    _client => $client,
...
sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinTecsUserBackofficeHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current OutputListOfTransactionsHistory data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinTecsUserBackofficeHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of OutputListOfTransactionsHistory fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (OutputListOfTransactionsHistoryCreateData shape). ctrl: optional
# per-call control. Returns the created OutputListOfTransactionsHistory data (hashref); dies
# with BluefinTecsUserBackofficeError on failure.
sub create {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      if ($result->{resdata}) {
        $self->{_data} = BluefinTecsUserBackofficeHelpers::to_map(
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinTecsUserBackofficeHelpers::to_map($callopts) || {};
  my $signal = BluefinTecsUserBackofficeHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinTecsUserBackofficeHelpers::to_map(
    BluefinTecsUserBackofficeHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinTecsUserBackofficeHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinTecsUserBackofficeHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinTecsUserBackofficeHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
