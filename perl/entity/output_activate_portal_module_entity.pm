# BluefinTecsUserBackoffice SDK OutputActivatePortalModule entity
...
package OutputActivatePortalModuleEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_activate_portal_module',
    _client => $client,
...
sub make {
  ...
  return OutputActivatePortalModuleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputActivatePortalModule data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputActivatePortalModule fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputActivatePortalModule.
#
# reqdata: body data hashref (OutputActivatePortalModuleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputActivatePortalModule data (hashref); dies
