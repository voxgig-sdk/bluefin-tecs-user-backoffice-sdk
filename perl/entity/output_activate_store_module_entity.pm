# BluefinTecsUserBackoffice SDK OutputActivateStoreModule entity
...
package OutputActivateStoreModuleEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_activate_store_module',
    _client => $client,
...
sub make {
  ...
  return OutputActivateStoreModuleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputActivateStoreModule data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputActivateStoreModule fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputActivateStoreModule.
#
# reqdata: body data hashref (OutputActivateStoreModuleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputActivateStoreModule data (hashref); dies
