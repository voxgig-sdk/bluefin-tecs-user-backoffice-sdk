...
package OutputChangeLogoEntity;
...
sub make {
  ...
  return OutputChangeLogoEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputChangeLogo data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputChangeLogo fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputChangeLogo.
#
# reqdata: body data hashref (OutputChangeLogoCreateData shape). ctrl: optional
# per-call control. Returns the created OutputChangeLogo data (hashref); dies
