# BluefinTecsUserBackoffice SDK OutputUpdateConsumer entity
...
package OutputUpdateConsumerEntity;
...
sub make {
  ...
  return OutputUpdateConsumerEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputUpdateConsumer data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputUpdateConsumer fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputUpdateConsumer.
#
# reqdata: body data hashref (OutputUpdateConsumerCreateData shape). ctrl: optional
# per-call control. Returns the created OutputUpdateConsumer data (hashref); dies
