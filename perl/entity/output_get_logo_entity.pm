# BluefinTecsUserBackoffice SDK OutputGetLogo entity
...
sub make {
  ...
  return OutputGetLogoEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputGetLogo data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputGetLogo fields).
sub match_get {
  my ($self) = @_;
...
# Load a single OutputGetLogo.
#
# reqmatch: match criteria hashref (id/query fields; OutputGetLogoLoadMatch
...
# Returns the loaded OutputGetLogo data (hashref); dies with BluefinTecsUserBackofficeError
# on failure.
sub load {
