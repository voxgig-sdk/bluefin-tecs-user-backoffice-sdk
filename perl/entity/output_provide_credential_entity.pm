# BluefinTecsUserBackoffice SDK OutputProvideCredential entity
...
package OutputProvideCredentialEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'output_provide_credential',
    _client => $client,
...
sub make {
  ...
  return OutputProvideCredentialEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputProvideCredential data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputProvideCredential fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputProvideCredential.
#
# reqdata: body data hashref (OutputProvideCredentialCreateData shape). ctrl: optional
# per-call control. Returns the created OutputProvideCredential data (hashref); dies
