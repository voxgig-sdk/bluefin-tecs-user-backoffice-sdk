# BluefinTecsUserBackoffice SDK OutputRemoveRole entity
...
package OutputRemoveRoleEntity;
...
sub make {
  ...
  return OutputRemoveRoleEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputRemoveRole data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputRemoveRole fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputRemoveRole.
#
# reqdata: body data hashref (OutputRemoveRoleCreateData shape). ctrl: optional
# per-call control. Returns the created OutputRemoveRole data (hashref); dies
