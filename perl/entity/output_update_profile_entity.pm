# BluefinTecsUserBackoffice SDK OutputUpdateProfile entity
...
package OutputUpdateProfileEntity;
...
sub make {
  ...
  return OutputUpdateProfileEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputUpdateProfile data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputUpdateProfile fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputUpdateProfile.
#
# reqdata: body data hashref (OutputUpdateProfileCreateData shape). ctrl: optional
# per-call control. Returns the created OutputUpdateProfile data (hashref); dies
