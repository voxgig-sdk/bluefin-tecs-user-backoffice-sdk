...
sub new {
  ...
  my $config = BluefinTecsUserBackofficeConfig::shared_config();
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = BluefinTecsUserBackofficeHelpers::gpath($self->{options}, 'allow.op');
...
sub graphql {
  ...
  my $errors = BluefinTecsUserBackofficeHelpers::gpath($res, 'data.errors');
