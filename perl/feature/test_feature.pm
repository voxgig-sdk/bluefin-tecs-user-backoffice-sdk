...
package BluefinTecsUserBackofficeTestFeature;
...
sub init {
  ...
  my $entity = BluefinTecsUserBackofficeHelpers::gp($options, 'entity');
  ...
  my $envelope = sub {
    ...
    my $tm = BluefinTecsUserBackofficeHelpers::gp($fctx->{point}, 'transform');
    my $spec = BluefinTecsUserBackofficeHelpers::gp($tm, 'res');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinTecsUserBackofficeHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinTecsUserBackofficeHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinTecsUserBackofficeHelpers::ge($found, 0);
      return $respond->($fctx, 404, undef, { 'statusText' => 'Not found' })
        unless BluefinTecsUserBackofficeHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinTecsUserBackofficeHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinTecsUserBackofficeHelpers::gp($ent, 'id');
  ...
  my $net = BluefinTecsUserBackofficeHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinTecsUserBackofficeHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinTecsUserBackofficeHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinTecsUserBackofficeHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinTecsUserBackofficeHelpers::ge($points, -1);

  my $params_path = BluefinTecsUserBackofficeHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinTecsUserBackofficeHelpers::gp($op->{alias}, $key) : undef;
