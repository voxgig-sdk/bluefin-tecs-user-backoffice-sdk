# BluefinTecsUserBackoffice SDK paging feature
#
...
package BluefinTecsUserBackofficePagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinTecsUserBackofficeHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (BluefinTecsUserBackofficeHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = BluefinTecsUserBackofficeHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = BluefinTecsUserBackofficeHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = BluefinTecsUserBackofficeHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = BluefinTecsUserBackofficeHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = BluefinTecsUserBackofficeHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = BluefinTecsUserBackofficeHelpers::gpath($conn, $morepath);
      if (BluefinTecsUserBackofficeHelpers::is_true($more) || BluefinTecsUserBackofficeHelpers::is_false($more)) {
        $paging->{hasMore} = BluefinTecsUserBackofficeHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinTecsUserBackofficeHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinTecsUserBackofficeHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinTecsUserBackofficeHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinTecsUserBackofficeHelpers::is_true($bhasmore) || BluefinTecsUserBackofficeHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinTecsUserBackofficeHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
