...
package BluefinTecsUserBackofficeContext;
...
sub new {
  ...
  my $gcp = \&BluefinTecsUserBackofficeHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinTecsUserBackofficeControl->new();
  ...
  $self->{data} = BluefinTecsUserBackofficeHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinTecsUserBackofficeHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinTecsUserBackofficeHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinTecsUserBackofficeHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinTecsUserBackofficeSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinTecsUserBackofficeResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinTecsUserBackofficeResponse'))
...
sub resolve_op {
  ...
  return BluefinTecsUserBackofficeOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinTecsUserBackofficeHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinTecsUserBackofficeHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinTecsUserBackofficeOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinTecsUserBackofficeError->new($code, $msg, $self);
}
