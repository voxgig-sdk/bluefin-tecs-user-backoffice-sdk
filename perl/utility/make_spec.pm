# BluefinTecsUserBackoffice SDK utility: make_spec
...
$REGISTRY{make_spec} = sub {
  ...
  my $base = BluefinTecsUserBackofficeHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = BluefinTecsUserBackofficeHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = BluefinTecsUserBackofficeHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = BluefinTecsUserBackofficeHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = BluefinTecsUserBackofficeHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = BluefinTecsUserBackofficeHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $BluefinTecsUserBackofficeUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
