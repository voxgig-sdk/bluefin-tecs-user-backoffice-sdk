...
$BluefinTecsUserBackofficeUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinTecsUserBackofficeUtilities::REGISTRY) {
    $u->{$k} = $BluefinTecsUserBackofficeUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinTecsUserBackofficeHelpers::struct_facade();
};
