# BluefinTecsUserBackoffice SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinTecsUserBackofficeHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinTecsUserBackofficeHelpers::gpath($options, 'auth.prefix');
