# BluefinTecsUserBackoffice SDK utility: make_point
...
sub _parts_len {
  my ($point) = @_;
  my $parts = BluefinTecsUserBackofficeHelpers::gp($point, 'parts');
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinTecsUserBackofficeError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinTecsUserBackofficeHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    my $matched = 0;
    for my $p (@{ $op->{points} }) {
      my $select_def = BluefinTecsUserBackofficeHelpers::to_map(BluefinTecsUserBackofficeHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinTecsUserBackofficeHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinTecsUserBackofficeHelpers::gp($reqselector, "$ek");
            my $sv = BluefinTecsUserBackofficeHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinTecsUserBackofficeHelpers::gp($reqselector, '$action');
        my $select_action = BluefinTecsUserBackofficeHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinTecsUserBackofficeHelpers::eqv($req_action, $select_action);
      }
    ...
    unless ($matched) {
      ...
      my $unmatched_action = $reqselector
        ? BluefinTecsUserBackofficeHelpers::gp($reqselector, '$action') : undef;
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinTecsUserBackofficeHelpers::to_map(BluefinTecsUserBackofficeHelpers::gp($point, 'select'));
        my $point_action = BluefinTecsUserBackofficeHelpers::gp($point_select, '$action');
        unless (BluefinTecsUserBackofficeHelpers::eqv($req_action, $point_action)) {
          return (undef, $ctx->make_error('point_action_invalid',
            "Operation \"$op->{name}\" action \""
