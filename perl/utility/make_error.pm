...
$REGISTRY{make_error} = sub {
  my ($ctx, $err) = @_;
  if (!defined $ctx) {
    $ctx = BluefinTecsUserBackofficeContext->new({}, undef);
  }
  my $op = $ctx->{op} || BluefinTecsUserBackofficeOperation->new({});
  my $opname = $op->{name};
  $opname = 'unknown operation' if !defined $opname || '' eq $opname || '_' eq $opname;

  my $result = $ctx->{result} || BluefinTecsUserBackofficeResult->new({});
  $result->{ok} = 0;

  $err = $result->{err} if !defined $err;
  $err = $ctx->make_error('unknown', 'unknown error') if !defined $err;

  my $errmsg = (Scalar::Util::blessed($err) && $err->isa('BluefinTecsUserBackofficeError'))
    ? $err->{msg} : "$err";
  $errmsg =~ s/\s+\z//;
  my $msg = "BluefinTecsUserBackofficeSDK: $opname: $errmsg";
  ...
  my $sdk_err = BluefinTecsUserBackofficeError->new('', $msg, $ctx);
