# BluefinTecsUserBackoffice SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinTecsUserBackofficeHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinTecsUserBackofficeHelpers::gp($options, 'extend');
  ...
  my @featureorder;
  my $feature_raw = BluefinTecsUserBackofficeHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinTecsUserBackofficeHelpers::gp($entry, 'name');
  ...
  BluefinTecsUserBackofficeHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinTecsUserBackofficeHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinTecsUserBackofficeConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinTecsUserBackofficeHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinTecsUserBackofficeHelpers::to_map(
      BluefinTecsUserBackofficeHelpers::gp($opts, 'feature')) || {};
