# BluefinTecsUserBackoffice SDK utility: prepare_query
...
$REGISTRY{prepare_query} = sub {
  ...
  my $params = [];
  if ($point) {
    my $p = BluefinTecsUserBackofficeHelpers::gp($point, 'params');
  ...
  if ($items) {
    for my $item (@$items) {
      my ($key, $val) = @$item;
      next unless BluefinTecsUserBackofficeHelpers::rb_truthy($val) && defined $key && !ref $key;
