...
class OutputGetLogoDirectTest < Minitest::Test
  def test_direct_load_output_get_logo
    setup = output_get_logo_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-output_get_logo", setup[:live] ? "live" : "unit")
    if _should_skip
...
def output_get_logo_direct_setup(mockres)
