...
class OutputActivatePortalModuleEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_PORTAL_MODULE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    output_activate_portal_module_ref01_data = Helpers.to_map(output_activate_portal_module_ref01_data_result.respond_to?(:data_get) ? output_activate_portal_module_ref01_data_result.data_get : output_activate_portal_module_ref01_data_result)
...
def output_activate_portal_module_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_PORTAL_MODULE_ENTID"]
