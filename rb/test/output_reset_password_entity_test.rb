...
class OutputResetPasswordEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESET_PASSWORD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    output_reset_password_ref01_data = Helpers.to_map(output_reset_password_ref01_data_result.respond_to?(:data_get) ? output_reset_password_ref01_data_result.data_get : output_reset_password_ref01_data_result)
...
def output_reset_password_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESET_PASSWORD_ENTID"]
