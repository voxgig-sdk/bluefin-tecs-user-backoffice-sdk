...
class OutputListOfTransactionsHistoryEntity
  ...
  def make
    opts = @_entopts.dup
    OutputListOfTransactionsHistoryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinTecsUserBackofficeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [OutputListOfTransactionsHistory, Hash] the current OutputListOfTransactionsHistory data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinTecsUserBackofficeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of OutputListOfTransactionsHistory fields)
  def match_get
  ...
  #
  # @param reqdata [OutputListOfTransactionsHistoryCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [OutputListOfTransactionsHistory, Hash] the created OutputListOfTransactionsHistory; raises BluefinTecsUserBackofficeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = BluefinTecsUserBackofficeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
