...
use bluefin_tecs_user_backoffice_sdk::core::helpers::{get_str, getp, ja, jo, setp, to_map, vfn};
...
use bluefin_tecs_user_backoffice_sdk::{
    ...
    BluefinTecsUserBackofficeError, BluefinTecsUserBackofficeSDK, SdkResult, Spec, Utility, Value,
};
...
where
    F: FnMut(&mut Vec<Value>) -> Result<Value, BluefinTecsUserBackofficeError> + 'static,
{
...
fn base_client() -> (Rc<BluefinTecsUserBackofficeSDK>, Rc<Utility>) {
...
fn make_test_ctx(client: &Rc<BluefinTecsUserBackofficeSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn make_test_full_ctx(client: &Rc<BluefinTecsUserBackofficeSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn primary_fetcher_live() {
    ...
    let c = calls.clone();
    let live_client = BluefinTecsUserBackofficeSDK::new(jo(vec![(
        "system",
        jo(vec![(
            "fetch",
            vfn(move |args| {
...
fn primary_fetcher_blocked_test_mode() {
    ...
    let blocked_client = BluefinTecsUserBackofficeSDK::new(jo(vec![(
        "system",
