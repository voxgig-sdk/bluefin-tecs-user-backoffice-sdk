...
use bluefin_tecs_user_backoffice_sdk::core::helpers::{getp, getpath, ja, jo};
...
use bluefin_tecs_user_backoffice_sdk::{
    ...
    BluefinTecsUserBackofficeSDK, Response, SdkResult, Spec, Utility, Value,
};

// plClient builds a client + isolated utility for pipeline utility tests.
fn pl_client(sdkopts: Value) -> (Rc<BluefinTecsUserBackofficeSDK>, Rc<Utility>) {
...
fn util_with(client: &Rc<BluefinTecsUserBackofficeSDK>, fetcher: FetcherFn) -> Rc<Utility> {
...
fn pipeline_feature_add_ordering_before_after_replace() {
    ...
    let names = |client: &Rc<BluefinTecsUserBackofficeSDK>| -> String {
        client
            .features
            .borrow()
            .iter()
...
use bluefin_tecs_user_backoffice_sdk::core::helpers::get_str;
