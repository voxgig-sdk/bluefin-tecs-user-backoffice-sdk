...
#[test]
fn output_get_logo_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.output_get_logo(Value::Noval);
    assert_eq!(ent.get_name(), "output_get_logo");
}

#[test]
fn output_get_logo_entity_stream() {
    ...
    let plainent = plainsdk.output_get_logo(Value::Noval);
...
#[test]
fn output_get_logo_entity_basic() {
    let setup = output_get_logo_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("output_get_logo.{}", op), mode);
        if skip {
    ...
    let output_get_logo_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "output_get_logo"], &setup.data)));
    let output_get_logo_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&output_get_logo_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let output_get_logo_ref01_ent = client.output_get_logo(Value::Noval);
    let output_get_logo_ref01_match_dt0 = Value::empty_map();
...
fn output_get_logo_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("output_get_logo");
    entity_data_file.push("OutputGetLogoTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("output_get_logo01"), Value::str("output_get_logo02"), Value::str("output_get_logo03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
