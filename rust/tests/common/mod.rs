...
/// env_override (mirrors go): when BLUEFINTECSUSERBACKOFFICE_TEST_LIVE or
/// BLUEFINTECSUSERBACKOFFICE_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("BLUEFINTECSUSERBACKOFFICE_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("BLUEFINTECSUSERBACKOFFICE_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("BLUEFINTECSUSERBACKOFFICE_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "BLUEFINTECSUSERBACKOFFICE_TEST_EXPLAIN", Value::str(explain));
        }
    }
