...
/// BluefinTecsUserBackofficeEntity: the full CRUD contract every generated entity
/// implements. Ops the API spec doesn't define are runtime-error stubs.
pub trait BluefinTecsUserBackofficeEntity: Entity + Sized {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinTecsUserBackofficeError>;
    fn create(self: &Rc<Self>, reqdata: Value, ctrl: Value) -> Result<Rc<Self>, BluefinTecsUserBackofficeError>;
    fn update(self: &Rc<Self>, reqdata: Value, ctrl: Value) -> Result<Rc<Self>, BluefinTecsUserBackofficeError>;
    fn remove(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Rc<Self>, BluefinTecsUserBackofficeError>;
}

/// Transport: `(ctx, url, fetchdef) -> transport-shaped response map`.
/// An Ok(Value::Noval) mirrors go's (nil, nil) transport result.
pub type FetcherFn =
    Rc<dyn Fn(&Rc<Context>, &str, &Value) -> Result<Value, BluefinTecsUserBackofficeError>>;
...
pub enum OutVal {
    Val(Value),
    Err(BluefinTecsUserBackofficeError),
