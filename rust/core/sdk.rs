...
impl BluefinTecsUserBackofficeSDK {
    ...
    fn op_denied(&self, op: &str) -> Value {
        ...
        jo(vec![
            ...
            ("err", Value::str(format!(
                "BluefinTecsUserBackofficeSDK: {}: operation not allowed by SDK option \
    ...
    // gate by passing it.
    fn raw_request(&self, fetchargs: Value) -> Result<Value, BluefinTecsUserBackofficeError> {
        let utility = &self.utility;
    ...
    pub fn graphql(
        ...
        if let Value::List(items) = &errors {
            ...
            if let Some(first) = first {
                ...
                setp(&res, "err",
                     Value::str(format!("BluefinTecsUserBackofficeSDK: graphql: {}", msg)));
