...
impl BluefinTecsUserBackofficeEntity for OutputGetLogoEntity {
    ...
    fn load(&self, reqmatch: Value, ctrl: Value) -> Result<Value, BluefinTecsUserBackofficeError> {
        let ctx = self.utility.make_context(
            CtxSpec {
    ...
    fn create(&self, _reqdata: Value, _ctrl: Value) -> Result<Value, BluefinTecsUserBackofficeError> {
