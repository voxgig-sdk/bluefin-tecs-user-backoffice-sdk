// OutputChangeLogo entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputChangeLogoEntity = struct {
    name: []const u8 = "output_change_logo",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputChangeLogoEntity {
        ...
        const e = h.A().create(OutputChangeLogoEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputChangeLogoEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputChangeLogoEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputChangeLogoEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputChangeLogoEntity, ctx: *Context, post_done: *const fn (*OutputChangeLogoEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputChangeLogoEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputChangeLogoEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputChangeLogoEntity, ctx: *Context, post_done: *const fn (*OutputChangeLogoEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputChangeLogoEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputChangeLogoEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputChangeLogoEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputChangeLogoEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputChangeLogoEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputChangeLogoEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputChangeLogoEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputChangeLogoEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputChangeLogoEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputChangeLogoEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputChangeLogoEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputChangeLogoEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
