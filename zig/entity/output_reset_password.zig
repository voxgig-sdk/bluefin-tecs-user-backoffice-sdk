// OutputResetPassword entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputResetPasswordEntity = struct {
    name: []const u8 = "output_reset_password",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputResetPasswordEntity {
        ...
        const e = h.A().create(OutputResetPasswordEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputResetPasswordEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputResetPasswordEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputResetPasswordEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputResetPasswordEntity, ctx: *Context, post_done: *const fn (*OutputResetPasswordEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputResetPasswordEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputResetPasswordEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputResetPasswordEntity, ctx: *Context, post_done: *const fn (*OutputResetPasswordEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputResetPasswordEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputResetPasswordEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputResetPasswordEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputResetPasswordEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputResetPasswordEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputResetPasswordEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputResetPasswordEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputResetPasswordEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputResetPasswordEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputResetPasswordEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputResetPasswordEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputResetPasswordEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
