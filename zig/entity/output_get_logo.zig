// OutputGetLogo entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputGetLogoEntity = struct {
    name: []const u8 = "output_get_logo",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputGetLogoEntity {
        ...
        const e = h.A().create(OutputGetLogoEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputGetLogoEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputGetLogoEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputGetLogoEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputGetLogoEntity, ctx: *Context, post_done: *const fn (*OutputGetLogoEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputGetLogoEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputGetLogoEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputGetLogoEntity, ctx: *Context, post_done: *const fn (*OutputGetLogoEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputGetLogoEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputGetLogoEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputGetLogoEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputGetLogoEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputGetLogoEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputGetLogoEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputGetLogoEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *OutputGetLogoEntity, ctx: *Context) void {
    ...
    pub fn list(self: *OutputGetLogoEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *OutputGetLogoEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *OutputGetLogoEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputGetLogoEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
