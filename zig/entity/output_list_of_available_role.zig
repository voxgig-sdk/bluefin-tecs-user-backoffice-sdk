// OutputListOfAvailableRole entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *OutputListOfAvailableRoleEntity,
...
pub const OutputListOfAvailableRoleEntity = struct {
    name: []const u8 = "output_list_of_available_role",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputListOfAvailableRoleEntity {
        ...
        const e = h.A().create(OutputListOfAvailableRoleEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputListOfAvailableRoleEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputListOfAvailableRoleEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputListOfAvailableRoleEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputListOfAvailableRoleEntity, ctx: *Context, post_done: *const fn (*OutputListOfAvailableRoleEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputListOfAvailableRoleEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputListOfAvailableRoleEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputListOfAvailableRoleEntity, ctx: *Context, post_done: *const fn (*OutputListOfAvailableRoleEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputListOfAvailableRoleEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputListOfAvailableRoleEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputListOfAvailableRoleEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputListOfAvailableRoleEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputListOfAvailableRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputListOfAvailableRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputListOfAvailableRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputListOfAvailableRoleEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputListOfAvailableRoleEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputListOfAvailableRoleEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputListOfAvailableRoleEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputListOfAvailableRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
