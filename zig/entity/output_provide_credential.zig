// OutputProvideCredential entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *OutputProvideCredentialEntity,
...
pub const OutputProvideCredentialEntity = struct {
    name: []const u8 = "output_provide_credential",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputProvideCredentialEntity {
        ...
        const e = h.A().create(OutputProvideCredentialEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputProvideCredentialEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputProvideCredentialEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputProvideCredentialEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputProvideCredentialEntity, ctx: *Context, post_done: *const fn (*OutputProvideCredentialEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputProvideCredentialEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputProvideCredentialEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputProvideCredentialEntity, ctx: *Context, post_done: *const fn (*OutputProvideCredentialEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputProvideCredentialEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputProvideCredentialEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputProvideCredentialEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputProvideCredentialEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputProvideCredentialEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputProvideCredentialEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputProvideCredentialEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputProvideCredentialEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputProvideCredentialEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputProvideCredentialEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputProvideCredentialEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputProvideCredentialEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
