// OutputResendLink entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputResendLinkEntity = struct {
    name: []const u8 = "output_resend_link",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputResendLinkEntity {
        ...
        const e = h.A().create(OutputResendLinkEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputResendLinkEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputResendLinkEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputResendLinkEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputResendLinkEntity, ctx: *Context, post_done: *const fn (*OutputResendLinkEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputResendLinkEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputResendLinkEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputResendLinkEntity, ctx: *Context, post_done: *const fn (*OutputResendLinkEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputResendLinkEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputResendLinkEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputResendLinkEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputResendLinkEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputResendLinkEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputResendLinkEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputResendLinkEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputResendLinkEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputResendLinkEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputResendLinkEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputResendLinkEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputResendLinkEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
