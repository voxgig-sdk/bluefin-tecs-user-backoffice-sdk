// OutputCreateServiceUser entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *OutputCreateServiceUserEntity,
...
pub const OutputCreateServiceUserEntity = struct {
    name: []const u8 = "output_create_service_user",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputCreateServiceUserEntity {
        ...
        const e = h.A().create(OutputCreateServiceUserEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputCreateServiceUserEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputCreateServiceUserEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputCreateServiceUserEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputCreateServiceUserEntity, ctx: *Context, post_done: *const fn (*OutputCreateServiceUserEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputCreateServiceUserEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputCreateServiceUserEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputCreateServiceUserEntity, ctx: *Context, post_done: *const fn (*OutputCreateServiceUserEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputCreateServiceUserEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputCreateServiceUserEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputCreateServiceUserEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputCreateServiceUserEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputCreateServiceUserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputCreateServiceUserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputCreateServiceUserEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputCreateServiceUserEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputCreateServiceUserEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputCreateServiceUserEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputCreateServiceUserEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputCreateServiceUserEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
