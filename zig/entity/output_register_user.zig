// OutputRegisterUser entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputRegisterUserEntity = struct {
    name: []const u8 = "output_register_user",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputRegisterUserEntity {
        ...
        const e = h.A().create(OutputRegisterUserEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputRegisterUserEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputRegisterUserEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputRegisterUserEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputRegisterUserEntity, ctx: *Context, post_done: *const fn (*OutputRegisterUserEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputRegisterUserEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputRegisterUserEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputRegisterUserEntity, ctx: *Context, post_done: *const fn (*OutputRegisterUserEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputRegisterUserEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputRegisterUserEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputRegisterUserEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputRegisterUserEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputRegisterUserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputRegisterUserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputRegisterUserEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputRegisterUserEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputRegisterUserEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputRegisterUserEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputRegisterUserEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputRegisterUserEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
