// OutputCreateMandator entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputCreateMandatorEntity = struct {
    name: []const u8 = "output_create_mandator",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputCreateMandatorEntity {
        ...
        const e = h.A().create(OutputCreateMandatorEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputCreateMandatorEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputCreateMandatorEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputCreateMandatorEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputCreateMandatorEntity, ctx: *Context, post_done: *const fn (*OutputCreateMandatorEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputCreateMandatorEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputCreateMandatorEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputCreateMandatorEntity, ctx: *Context, post_done: *const fn (*OutputCreateMandatorEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputCreateMandatorEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputCreateMandatorEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputCreateMandatorEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputCreateMandatorEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputCreateMandatorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputCreateMandatorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputCreateMandatorEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputCreateMandatorEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputCreateMandatorEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputCreateMandatorEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputCreateMandatorEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputCreateMandatorEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
