// OutputRemoveRole entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputRemoveRoleEntity = struct {
    name: []const u8 = "output_remove_role",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputRemoveRoleEntity {
        ...
        const e = h.A().create(OutputRemoveRoleEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputRemoveRoleEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputRemoveRoleEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputRemoveRoleEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputRemoveRoleEntity, ctx: *Context, post_done: *const fn (*OutputRemoveRoleEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputRemoveRoleEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputRemoveRoleEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputRemoveRoleEntity, ctx: *Context, post_done: *const fn (*OutputRemoveRoleEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputRemoveRoleEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputRemoveRoleEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputRemoveRoleEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputRemoveRoleEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputRemoveRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputRemoveRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputRemoveRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputRemoveRoleEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputRemoveRoleEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputRemoveRoleEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputRemoveRoleEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputRemoveRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
