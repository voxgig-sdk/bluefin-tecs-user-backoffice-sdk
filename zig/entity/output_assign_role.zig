// OutputAssignRole entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputAssignRoleEntity = struct {
    name: []const u8 = "output_assign_role",
    ...
    pub fn new(client: *sdk.BluefinTecsUserBackofficeSDK, entopts_in: Value) *OutputAssignRoleEntity {
        ...
        const e = h.A().create(OutputAssignRoleEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputAssignRoleEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputAssignRoleEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputAssignRoleEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputAssignRoleEntity, ctx: *Context, post_done: *const fn (*OutputAssignRoleEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputAssignRoleEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputAssignRoleEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputAssignRoleEntity, ctx: *Context, post_done: *const fn (*OutputAssignRoleEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputAssignRoleEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputAssignRoleEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputAssignRoleEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputAssignRoleEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputAssignRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputAssignRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputAssignRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputAssignRoleEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputAssignRoleEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputAssignRoleEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputAssignRoleEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputAssignRoleEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
