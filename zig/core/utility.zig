...
pub fn make_error_util(ctx: *Context) E!Value {
    ...
    const the_err: *err.BluefinTecsUserBackofficeError = in_err orelse (result.err orelse ctx.make_error("unknown", "unknown error"));

    const errmsg = the_err.msg;
    const msg0 = fmt("BluefinTecsUserBackofficeSDK: {s}: {s}", .{ opname, errmsg });
    ...
    const sdk_err = err.BluefinTecsUserBackofficeError.make("", msg);
