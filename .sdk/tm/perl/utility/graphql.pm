# BluefinTecsUserBackoffice SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinTecsUserBackofficeHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinTecsUserBackofficeHelpers::gp($ext, 'code');
  $raw = BluefinTecsUserBackofficeHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinTecsUserBackofficeHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinTecsUserBackofficeHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinTecsUserBackofficeHelpers::gp($spec, 'name');
    my $from = BluefinTecsUserBackofficeHelpers::gp($spec, 'from');
    ...
    my $val = BluefinTecsUserBackofficeHelpers::gp($reqsrc, $from);
    $val = BluefinTecsUserBackofficeHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinTecsUserBackofficeHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinTecsUserBackofficeHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinTecsUserBackofficeHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinTecsUserBackofficeHelpers::gp($first, 'message');
