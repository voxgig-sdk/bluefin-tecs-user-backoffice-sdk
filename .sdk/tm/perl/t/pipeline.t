...
sub resp {
  ...
  return BluefinTecsUserBackofficeResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinTecsUserBackofficeSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinTecsUserBackofficeError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinTecsUserBackofficeResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinTecsUserBackofficeError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinTecsUserBackofficeError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinTecsUserBackofficeSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinTecsUserBackofficeBaseFeature->new;
  my $b = BluefinTecsUserBackofficeBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinTecsUserBackofficeBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinTecsUserBackofficeHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinTecsUserBackofficeHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinTecsUserBackofficeContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinTecsUserBackofficeSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsUserBackofficeResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinTecsUserBackofficeResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsUserBackofficeResponse->new({
