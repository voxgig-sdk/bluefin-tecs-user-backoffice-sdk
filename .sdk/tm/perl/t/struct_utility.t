...
my $runner = BluefinTecsUserBackofficeOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinTecsUserBackofficeOmni::NULLMARK();
...
{
  my $td = BluefinTecsUserBackofficeOmni::tostruct(
...
{
  my $basic = BluefinTecsUserBackofficeOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinTecsUserBackofficeOmni::null_modifier }) });
