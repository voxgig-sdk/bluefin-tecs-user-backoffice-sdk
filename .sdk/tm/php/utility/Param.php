...
class BluefinTecsUserBackofficeParam
{
    public static function call(BluefinTecsUserBackofficeContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = BluefinTecsUserBackofficeHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
