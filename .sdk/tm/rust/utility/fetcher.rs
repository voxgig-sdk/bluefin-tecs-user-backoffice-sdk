...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinTecsUserBackofficeError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinTecsUserBackofficeSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinTecsUserBackofficeError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinTecsUserBackofficeError::new("fetch_body", &format!("{}", e)))?;
