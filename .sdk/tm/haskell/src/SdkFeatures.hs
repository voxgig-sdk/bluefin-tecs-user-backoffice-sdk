-- BluefinTecsUserBackoffice SDK features + API-agnostic client helpers.
--
...
clienttrackFeature = do
  ...
  let nameOf = do opts <- readIORef options; nm <- optStr opts "clientName" "BluefinTecsUserBackoffice-SDK"; ver <- optStr opts "clientVersion" "0.0.1"; pure (nm ++ "/" ++ ver)
      genId kind = do
        opts <- readIORef options; v <- getp opts "idgen"
...
opDenied client op = do
  ...
  jo [ ("ok", VBool False)
     , ("err", VStr ("BluefinTecsUserBackofficeSDK: " ++ op ++ ": operation not allowed by"
  ...
  if not allowed then opDenied client "graphql" else do
    ...
    case errors of
      ...
      (firsterr : _) -> do
        ...
        setp res "err" (VStr ("BluefinTecsUserBackofficeSDK: graphql: " ++ msg))
        setp res "graphql" ev
        pure res
