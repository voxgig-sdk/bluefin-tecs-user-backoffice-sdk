// BluefinTecsUserBackoffice SDK — drives the primary utility functions against the shared
...
static res::Run& primaryRun() {
  static res::Run run =
      res::makeRunner(TEST_JSON_FILE, BluefinTecsUserBackofficeSDK::testSDK())
          .runner("primary", Value::undef());
  return run;
}

static std::shared_ptr<BluefinTecsUserBackofficeSDK> client() { return BluefinTecsUserBackofficeSDK::testSDK(); }
...
static CtxPtr makeTestCtx(std::shared_ptr<BluefinTecsUserBackofficeSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<BluefinTecsUserBackofficeSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<BluefinTecsUserBackofficeSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<BluefinTecsUserBackofficeSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = BluefinTecsUserBackofficeSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = BluefinTecsUserBackofficeSDK::testSDK(Value::undef(), setupOpts);
