// BluefinTecsUserBackoffice SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinTecsUserBackofficeSDK> plClient(const Value& sdkopts) {
  return BluefinTecsUserBackofficeSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinTecsUserBackofficeSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinTecsUserBackofficeSDK> client) {
  std::string out;
