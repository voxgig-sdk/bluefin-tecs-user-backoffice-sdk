...
module BluefinTecsUserBackofficeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinTecsUserBackofficeResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinTecsUserBackofficeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinTecsUserBackofficeResponse.new(fetched)
    else
