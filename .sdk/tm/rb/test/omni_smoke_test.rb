...
class OmniSmokeTest < Minitest::Test
  ...
  def smoke_pack
    runner = BluefinTecsUserBackofficeOmni.make_runner(SMOKE_SPEC, BluefinTecsUserBackofficeSDK.test(nil, nil))
    runner.call("smoke")
  end
  ...
  def test_runset_fails_a_wrong_result_with_omnierror
    pack = smoke_pack
    caught = assert_raises(BluefinTecsUserBackofficeOmni::OmniError) do
