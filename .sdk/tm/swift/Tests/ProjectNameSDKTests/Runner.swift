// BluefinTecsUserBackoffice SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/BluefinTecsUserBackofficeSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // BluefinTecsUserBackofficeSDKTests
  ...
  static func errCode(_ e: Error?) -> String {
    (e as? BluefinTecsUserBackofficeError)?.code ?? ""
  }
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: BluefinTecsUserBackofficeSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = BluefinTecsUserBackofficeError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: BluefinTecsUserBackofficeSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return BluefinTecsUserBackofficeError(code, msg, nil)
  }
}
