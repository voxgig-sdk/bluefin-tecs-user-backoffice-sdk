# BluefinTecsUserBackoffice SDK netsim test
#
...
defmodule BluefinTecsUserBackoffice.NetsimTest do
  use ExUnit.Case
  ...
  test "offline simulation fails the request" do
    sdk = BluefinTecsUserBackoffice.test(H.deep(%{"net" => %{"offline" => true}}))
    res = BluefinTecsUserBackoffice.direct(sdk, H.deep(%{"path" => "/ping"}))
    assert S.getprop(res, "ok") == false
  end

  test "fail status simulation surfaces the error status" do
    sdk = BluefinTecsUserBackoffice.test(H.deep(%{"net" => %{"failTimes" => 1, "failStatus" => 503}}))
    res = BluefinTecsUserBackoffice.direct(sdk, H.deep(%{"path" => "/ping"}))
    assert S.getprop(res, "ok") == false
  ...
  test "error times simulation yields a connection error" do
    sdk = BluefinTecsUserBackoffice.test(H.deep(%{"net" => %{"errorTimes" => 1}}))
    res = BluefinTecsUserBackoffice.direct(sdk, H.deep(%{"path" => "/ping"}))
    assert S.getprop(res, "ok") == false
  ...
  test "latency simulation delays the request" do
    delay = 60
    sdk = BluefinTecsUserBackoffice.test(H.deep(%{"net" => %{"latency" => delay}}))
    start = System.monotonic_time(:millisecond)
    BluefinTecsUserBackoffice.direct(sdk, H.deep(%{"path" => "/ping"}))
  ...
  test "injectable sleep keeps latency deterministic" do
    ...
    sdk =
      BluefinTecsUserBackoffice.test(H.deep(%{"net" => %{"latency" => 250, "sleep" => fn ms -> send(parent, {:slept, ms}) end}}))
