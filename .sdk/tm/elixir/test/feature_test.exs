...
# BluefinTecsUserBackoffice.FeatureHarness — the same hook order and short-circuit rules as
...
defmodule BluefinTecsUserBackoffice.FeatureTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsUserBackoffice.Helpers, as: H
  alias BluefinTecsUserBackoffice.FeatureHarness, as: FH
    ...
    if has?("retry") do
      ...
      {server, calls} =
        FH.recording_server(fn n, _fd ->
          if n < 3,
            do: {nil, BluefinTecsUserBackoffice.Error.new("boom", "boom")},
  ...
  test "retry exhausted transport error surfaces" do
    if has?("retry") do
      clock = FH.make_clock()
      {server, calls} = FH.recording_server(fn _n, _fd -> {nil, BluefinTecsUserBackoffice.Error.new("boom", "boom")} end)
