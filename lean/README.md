# BluefinTecsUserBackoffice Lean SDK



The Lean SDK for the BluefinTecsUserBackoffice API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/bluefin-tecs-user-backoffice-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "BLUEFIN_TECS_USER_BACKOFFICE").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Output_activate_digital_module.create sdk (← emptyMap) (← emptyMap)
