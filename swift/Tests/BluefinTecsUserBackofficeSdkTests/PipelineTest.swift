...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinTecsUserBackofficeSDK, Utility) {
    let client = BluefinTecsUserBackofficeSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinTecsUserBackofficeSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinTecsUserBackofficeError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinTecsUserBackofficeSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinTecsUserBackofficeError)?.code, "boom")
  }
