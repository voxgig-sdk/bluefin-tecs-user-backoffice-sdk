...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: BluefinTecsUserBackofficeSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: BluefinTecsUserBackofficeSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> BluefinTecsUserBackofficeSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: BluefinTecsUserBackofficeSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.client = { options in
      ...
      return sdkProvider(BluefinTecsUserBackofficeSDK.testSDK(nil, opts), true)
    }

    // Client options may reference the runner store.
    provider.inject = { options, store in
      let o = tovalue(options)
      _ = BluefinTecsUserBackofficeSdk.inject(o, tovalue(store))
      return tojson(o)
    }
    ...
    provider.errify = { err in
      if let sdkerr = err as? BluefinTecsUserBackofficeError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("BluefinTecsUserBackofficeError")),
  ...
  final class Run {
    ...
    let client: BluefinTecsUserBackofficeSDK

    private let pack: Omni.RunPack

    fileprivate init(_ pack: Omni.RunPack, _ client: BluefinTecsUserBackofficeSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: BluefinTecsUserBackofficeSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: BluefinTecsUserBackofficeSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: BluefinTecsUserBackofficeSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: BluefinTecsUserBackofficeSDK, _ utility: Utility)
    -> Context
  {
