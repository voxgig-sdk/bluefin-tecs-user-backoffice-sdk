...
// `Util` alone would resolve to the SDK's BluefinTecsUserBackofficeSdk.Util namespace,
...
public static class OmniResolver
{
        ...
        var provider = new Provider
        {
            ...
            Client = options =>
            {
                var opts = FixNums(options) as Dictionary<string, object?>
                    ?? new Dictionary<string, object?>();
                var sub = SdkProvider(BluefinTecsUserBackofficeSDK.TestSDK(null, opts));
                MarkDefProvider(sub);
                return sub;
            },
            ...
            Errify = err =>
            {
                if (err is BluefinTecsUserBackofficeError sdkerr)
                {
                    var outmap = new Dictionary<string, object?>
                    {
    ...
    /// </summary>
    public static Context OmniCtx(object? arg, BluefinTecsUserBackofficeSDK client, Utility utility)
    {
        ...
        if (ctxmap.TryGetValue("client", out var p) && p is Provider prov &&
            DefProviderClient(prov) is BluefinTecsUserBackofficeSDK live)
        {
            client = live;
