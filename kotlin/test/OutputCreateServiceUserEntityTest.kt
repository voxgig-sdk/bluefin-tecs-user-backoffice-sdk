...
class OutputCreateServiceUserEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputCreateServiceUserBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_create_service_user.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_SERVICE_USER_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val outputCreateServiceUserRef01Ent = client.outputCreateServiceUser(null)
    var outputCreateServiceUserRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_create_service_user"), "output_create_service_user_ref01")) ?: linkedMapOf())

    val outputCreateServiceUserRef01DataResult = outputCreateServiceUserRef01Ent.create(outputCreateServiceUserRef01Data, null)
    outputCreateServiceUserRef01Data = Helpers.toMapAny(if (outputCreateServiceUserRef01DataResult is SdkEntity) outputCreateServiceUserRef01DataResult.data() else outputCreateServiceUserRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputCreateServiceUserRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputCreateServiceUserBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_create_service_user test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_SERVICE_USER_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_SERVICE_USER_ENTID"] = idmap
