...
class OutputActivateDigitalModuleEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputActivateDigitalModuleBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_activate_digital_module.$op", mode)
    ...
    var outputActivateDigitalModuleRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
  ...
  companion object {
    fun outputActivateDigitalModuleBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_activate_digital_module test data: " + e.message, e)
      }
