...
class OutputAssignRoleEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputAssignRoleBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_assign_role.$op", mode)
    ...
    var outputAssignRoleRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_assign_role"), "output_assign_role_ref01")) ?: linkedMapOf())

    val outputAssignRoleRef01DataResult = outputAssignRoleRef01Ent.create(outputAssignRoleRef01Data, null)
    outputAssignRoleRef01Data = Helpers.toMapAny(if (outputAssignRoleRef01DataResult is SdkEntity) outputAssignRoleRef01DataResult.data() else outputAssignRoleRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputAssignRoleRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputAssignRoleBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_assign_role test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ASSIGN_ROLE_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ASSIGN_ROLE_ENTID"] = idmap
