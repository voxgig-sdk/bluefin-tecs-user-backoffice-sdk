...
class OutputListOfAvailableRoleEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputListOfAvailableRoleBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_list_of_available_role.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val outputListOfAvailableRoleRef01Ent = client.outputListOfAvailableRole(null)
    var outputListOfAvailableRoleRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_list_of_available_role"), "output_list_of_available_role_ref01")) ?: linkedMapOf())
    ...
    outputListOfAvailableRoleRef01Data = Helpers.toMapAny(if (outputListOfAvailableRoleRef01DataResult is SdkEntity) outputListOfAvailableRoleRef01DataResult.data() else outputListOfAvailableRoleRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputListOfAvailableRoleRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputListOfAvailableRoleBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_list_of_available_role test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID"] = idmap
