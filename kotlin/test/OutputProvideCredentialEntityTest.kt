...
class OutputProvideCredentialEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputProvideCredentialBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_provide_credential.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_PROVIDE_CREDENTIAL_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val outputProvideCredentialRef01Ent = client.outputProvideCredential(null)
    var outputProvideCredentialRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_provide_credential"), "output_provide_credential_ref01")) ?: linkedMapOf())

    val outputProvideCredentialRef01DataResult = outputProvideCredentialRef01Ent.create(outputProvideCredentialRef01Data, null)
    outputProvideCredentialRef01Data = Helpers.toMapAny(if (outputProvideCredentialRef01DataResult is SdkEntity) outputProvideCredentialRef01DataResult.data() else outputProvideCredentialRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputProvideCredentialRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputProvideCredentialBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_provide_credential test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_PROVIDE_CREDENTIAL_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_PROVIDE_CREDENTIAL_ENTID"] = idmap
