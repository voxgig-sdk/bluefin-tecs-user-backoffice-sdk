...
class TestOutputGetLogoDirect:

    def test_should_direct_load_output_get_logo(self):
        setup = _output_get_logo_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-output_get_logo", "live" if setup["live"] else "unit")
        if _skip:
...
def _output_get_logo_direct_setup(mockres):
