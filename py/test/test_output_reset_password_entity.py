...
class TestOutputResetPasswordEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsUserBackofficeSDK.test(None, None)
        ent = testsdk.OutputResetPassword(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _output_reset_password_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "output_reset_password." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESET_PASSWORD_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        output_reset_password_ref01_ent = client.OutputResetPassword(None)
        ...
        output_reset_password_ref01_data = helpers.to_map(runner.entity_data(output_reset_password_ref01_ent.create(output_reset_password_ref01_data, None)))
        assert output_reset_password_ref01_data is not None



def _output_reset_password_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/output_reset_password/OutputResetPasswordTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESET_PASSWORD_ENTID"))
