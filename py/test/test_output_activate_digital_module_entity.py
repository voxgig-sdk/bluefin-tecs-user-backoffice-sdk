...
class TestOutputActivateDigitalModuleEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsUserBackofficeSDK.test(None, None)
        ent = testsdk.OutputActivateDigitalModule(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _output_activate_digital_module_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "output_activate_digital_module." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        output_activate_digital_module_ref01_ent = client.OutputActivateDigitalModule(None)
...
def _output_activate_digital_module_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/output_activate_digital_module/OutputActivateDigitalModuleTestData.json")
