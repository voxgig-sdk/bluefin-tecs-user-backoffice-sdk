...
class TestOutputGetLogoEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsUserBackofficeSDK.test(None, None)
        ent = testsdk.OutputGetLogo(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _output_get_logo_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "output_get_logo." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_GET_LOGO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        output_get_logo_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.output_get_logo")))
        output_get_logo_ref01_data = None
        if len(output_get_logo_ref01_data_raw) > 0:
            output_get_logo_ref01_data = helpers.to_map(output_get_logo_ref01_data_raw[0][1])

        # LOAD
        output_get_logo_ref01_ent = client.OutputGetLogo(None)
        output_get_logo_ref01_match_dt0 = {}
        output_get_logo_ref01_data_dt0_loaded = output_get_logo_ref01_ent.load(output_get_logo_ref01_match_dt0, None)
        assert output_get_logo_ref01_data_dt0_loaded is not None



def _output_get_logo_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/output_get_logo/OutputGetLogoTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_GET_LOGO_ENTID"))
