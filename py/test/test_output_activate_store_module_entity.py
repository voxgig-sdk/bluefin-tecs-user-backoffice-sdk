    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_STORE_MODULE_ENTID JSON to run live")
        client = setup["client"]
        ...
        output_activate_store_module_ref01_data = helpers.to_map(runner.entity_data(output_activate_store_module_ref01_ent.create(output_activate_store_module_ref01_data, None)))
...
def _output_activate_store_module_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_STORE_MODULE_ENTID"))
