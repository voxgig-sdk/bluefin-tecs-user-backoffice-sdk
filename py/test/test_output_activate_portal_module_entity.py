...
class TestOutputActivatePortalModuleEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsUserBackofficeSDK.test(None, None)
        ent = testsdk.OutputActivatePortalModule(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _output_activate_portal_module_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "output_activate_portal_module." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_PORTAL_MODULE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        output_activate_portal_module_ref01_ent = client.OutputActivatePortalModule(None)
        ...
        output_activate_portal_module_ref01_data = helpers.to_map(runner.entity_data(output_activate_portal_module_ref01_ent.create(output_activate_portal_module_ref01_data, None)))
        assert output_activate_portal_module_ref01_data is not None



def _output_activate_portal_module_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/output_activate_portal_module/OutputActivatePortalModuleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_PORTAL_MODULE_ENTID"))
