# BluefinTecsUserBackoffice SDK test runner
...
class BluefinTecsUserBackofficeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinTecsUserBackofficeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinTecsUserBackofficeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinTecsUserBackofficeTestRunner.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_LIVE")
        override = BluefinTecsUserBackofficeTestRunner.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinTecsUserBackofficeTestRunner.getenv(key)
        ...
        explain = BluefinTecsUserBackofficeTestRunner.getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinTecsUserBackofficeTestRunner._test_control is not None:
            return BluefinTecsUserBackofficeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinTecsUserBackofficeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinTecsUserBackofficeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinTecsUserBackofficeTestRunner.load_test_control()
...
def env_override(m):
    return BluefinTecsUserBackofficeTestRunner.env_override(m)


def entity_data(v):
    return BluefinTecsUserBackofficeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinTecsUserBackofficeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinTecsUserBackofficeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinTecsUserBackofficeTestRunner.load_test_control()


def live_client_options():
    return BluefinTecsUserBackofficeTestRunner.live_client_options()


def live_delay_ms():
    return BluefinTecsUserBackofficeTestRunner.live_delay_ms()
