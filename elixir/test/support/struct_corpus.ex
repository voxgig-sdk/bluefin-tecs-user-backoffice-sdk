# Vendored struct corpus runner for the BluefinTecsUserBackoffice SDK.
#
...
defmodule BluefinTecsUserBackoffice.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinTecsUserBackoffice.test(nil, S.clone(setup)), else: BluefinTecsUserBackoffice.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinTecsUserBackoffice.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinTecsUserBackoffice.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinTecsUserBackoffice.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinTecsUserBackoffice.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
