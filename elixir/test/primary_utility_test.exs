...
defmodule BluefinTecsUserBackoffice.PrimaryUtilityTest do
  ...
  alias BluefinTecsUserBackoffice.{Context, Utility, Spec, Result}

  defp client, do: BluefinTecsUserBackoffice.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinTecsUserBackoffice.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinTecsUserBackoffice.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "prepare_auth omits the header when no apikey" do
    c = BluefinTecsUserBackoffice.new()
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinTecsUserBackoffice.test()
    opts = BluefinTecsUserBackoffice.options_map(c)
