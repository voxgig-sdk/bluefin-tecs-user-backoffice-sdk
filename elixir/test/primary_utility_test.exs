...
defmodule BluefinTecsUserBackoffice.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsUserBackoffice.Omni, as: O
  alias BluefinTecsUserBackoffice.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinTecsUserBackoffice.get_utility(sdk)
    rootctx = BluefinTecsUserBackoffice.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinTecsUserBackoffice.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinTecsUserBackoffice.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinTecsUserBackoffice.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinTecsUserBackoffice.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinTecsUserBackoffice.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinTecsUserBackoffice.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinTecsUserBackoffice.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinTecsUserBackoffice.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinTecsUserBackoffice.test(nil, setup_opts(spec, "prepareAuth"))
