...
defmodule BluefinTecsUserBackoffice.StreamTest do
  use ExUnit.Case
  ...
  defp has_feature?(name) do
    f = S.getprop(BluefinTecsUserBackoffice.Config.make_config(), "feature")
  ...
  defp entity(sdk, name), do: apply(BluefinTecsUserBackoffice, String.to_atom(name), [sdk, nil])
  defp entmod(name), do: Module.concat([BluefinTecsUserBackoffice.Entity, Macro.camelize(name)])
  ...
  test "entity stream() runs the pipeline and yields items" do
    ...
    if name == nil do
      ...
      sdk = BluefinTecsUserBackoffice.test(S.jm(["entity", seed(name)]), nil)
      items = do_stream(sdk, name, nil)
      assert length(items) == 3
      assert S.ismap(hd(items))

      # signal cancels iteration between yields.
      sdk2 = BluefinTecsUserBackoffice.test(S.jm(["entity", seed(name)]), nil)
      ...
      if has_feature?("streaming") do
        ...
        ssdk =
          BluefinTecsUserBackoffice.test(
