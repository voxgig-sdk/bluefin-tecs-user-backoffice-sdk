# BluefinTecsUserBackoffice SDK utility
#
...
defmodule BluefinTecsUserBackoffice.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinTecsUserBackoffice.Helpers, as: H
  alias BluefinTecsUserBackoffice.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinTecsUserBackofficeSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinTecsUserBackoffice.Config.shared_config) and merge uses its nested nodes as
      ...
      resolved =
        Regex.replace(~r/\{([A-Za-z0-9_]+)\}/, base, fn _match, name ->
          ...
          cond do
            ...
            true ->
              raise BluefinTecsUserBackoffice.Error,
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinTecsUserBackoffice.Error{}, err) -> err.msg
    ...
    msg = "BluefinTecsUserBackofficeSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = %BluefinTecsUserBackoffice.Error{
      code: "",
      msg: msg,
      sdk: "BluefinTecsUserBackoffice",
      ctx: ctx,
    ...
    sdk_err = if match?(%BluefinTecsUserBackoffice.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinTecsUserBackoffice.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinTecsUserBackoffice.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
