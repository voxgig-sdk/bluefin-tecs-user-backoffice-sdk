# bluefin-tecs-user-backoffice-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinTecsUserBackoffice SDK as
two agent tools — `bluefin-tecs-user-backoffice_list` and `bluefin-tecs-user-backoffice_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-tecs-user-backoffice-mcp)
make build
...
claude mcp add --scope user bluefin-tecs-user-backoffice \
  -- /absolute/path/to/bluefin-tecs-user-backoffice-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-tecs-user-backoffice-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-tecs-user-backoffice_list: first page of records
{ "entity": "output_activate_digital_module" }
{ "entity": "output_activate_digital_module", "query": { } }

// bluefin-tecs-user-backoffice_load: one record by id
{ "entity": "output_get_logo", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-tecs-user-backoffice-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-tecs-user-backoffice \
     -- "$PWD"/dist/*/bluefin-tecs-user-backoffice-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-tecs-user-backoffice_list` and `bluefin-tecs-user-backoffice_load` tools now appear
   in new sessions. Ask the agent to *"list output_activate_digital_module using bluefin-tecs-user-backoffice"*
   and it calls `bluefin-tecs-user-backoffice_list` with `{"entity":"output_activate_digital_module"}`.
...
### Call the `bluefin-tecs-user-backoffice_list` tool
...
### Call the `bluefin-tecs-user-backoffice_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-tecs-user-backoffice_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-tecs-user-backoffice_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./bluefin-tecs-user-backoffice-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-tecs-user-backoffice_load","arguments":{"entity":"output_get_logo","query":{"id":1}}}}'
```
