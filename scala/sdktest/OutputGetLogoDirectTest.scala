// Generated direct-call tests for the output_get_logo entity (mirrors the java
...
object OutputGetLogoDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-output_get_logo") {
      ...
      rep.eq("direct-load-output_get_logo.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-output_get_logo.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-output_get_logo.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-output_get_logo.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-output_get_logo.calls", 1, setup.calls.size())
    }
  }
}
