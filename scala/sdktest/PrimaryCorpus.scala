...
import voxgig.bluefintecsuserbackofficesdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefintecsuserbackofficesdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinTecsUserBackofficeSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinTecsUserBackofficeSDK)(f: (Context, Seq[Value]) => Value): Unit = {
  ...
  private def clientFor(name: String, shared: BluefinTecsUserBackofficeSDK): BluefinTecsUserBackofficeSDK =
    sectionSetup(sectionRun(name)) match {
      case m @ VMap(_) => BluefinTecsUserBackofficeSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinTecsUserBackofficeSDK.testSDK()
