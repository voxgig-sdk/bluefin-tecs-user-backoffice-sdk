...
import voxgig.bluefintecsuserbackofficesdk.core._
import voxgig.bluefintecsuserbackofficesdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "BLUEFIN_TECS_USER_BACKOFFICE_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: BluefinTecsUserBackofficeSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => BluefinTecsUserBackofficeSDK): BluefinTecsUserBackofficeSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): BluefinTecsUserBackofficeSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new BluefinTecsUserBackofficeSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): BluefinTecsUserBackofficeSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      BluefinTecsUserBackofficeSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: BluefinTecsUserBackofficeSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: BluefinTecsUserBackofficeSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: BluefinTecsUserBackofficeSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = BluefinTecsUserBackofficeSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = BluefinTecsUserBackofficeSDK.testSDK(null, null)
