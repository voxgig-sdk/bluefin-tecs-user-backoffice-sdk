...
inline Value env_override(Value m) {
  std::string live = getenv_local("BLUEFINTECSUSERBACKOFFICE_TEST_LIVE");
  std::string over = getenv_local("BLUEFINTECSUSERBACKOFFICE_TEST_OVERRIDE");
  ...
  std::string explain = getenv_local("BLUEFINTECSUSERBACKOFFICE_TEST_EXPLAIN");
  if (!explain.empty()) map_put(m, "BLUEFINTECSUSERBACKOFFICE_TEST_EXPLAIN", Value(explain));
  return m;
}
