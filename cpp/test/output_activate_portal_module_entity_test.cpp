...
static OutputActivatePortalModuleSetup output_activate_portal_module_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_ACTIVATE_PORTAL_MODULE_ENTID"));
...
static void output_activate_portal_module_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"output_activate_portal_module", vmap({
  ...
  auto se = strsdk->output_activate_portal_module();
  std::vector<Value> items = se->stream("list", Value::undef(), Value::undef());
  ASSERT_EQ((int)items.size(), 2, "stream yields both seeded items");

  auto plainsdk = BluefinTecsUserBackofficeSDK::testSDK(seed, Value::undef());
  auto pe = plainsdk->output_activate_portal_module();
...
static void output_activate_portal_module_entity_basic() {
  ...
  {
    Value output_activate_portal_module_ref01_data_result = output_activate_portal_module_ref01_ent->create(Struct::clone(output_activate_portal_module_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(output_activate_portal_module_entity_stream);
