...
static OutputUpdateProfileSetup output_update_profile_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_UPDATE_PROFILE_ENTID"));
...
static void output_update_profile_entity_basic() {
  ...
  {
    Value output_update_profile_ref01_data_result = output_update_profile_ref01_ent->create(Struct::clone(output_update_profile_ref01_data), Value::undef())->data();
