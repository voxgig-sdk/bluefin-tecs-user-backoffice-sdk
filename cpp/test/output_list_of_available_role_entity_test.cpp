...
static OutputListOfAvailableRoleSetup output_list_of_available_role_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID"));
...
static void output_list_of_available_role_entity_basic() {
  ...
  {
    Value output_list_of_available_role_ref01_data_result = output_list_of_available_role_ref01_ent->create(Struct::clone(output_list_of_available_role_ref01_data), Value::undef())->data();
