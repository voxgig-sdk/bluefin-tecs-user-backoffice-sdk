...
struct OutputActivateStoreModuleSetup {
...
static OutputActivateStoreModuleSetup output_activate_store_module_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/output_activate_store_module/OutputActivateStoreModuleTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("output_activate_store_module01"), Value("output_activate_store_module02"), Value("output_activate_store_module03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void output_activate_store_module_entity_instance() {
  auto testsdk = BluefinTecsUserBackofficeSDK::testSDK();
  auto ent = testsdk->output_activate_store_module();
...
static void output_activate_store_module_entity_basic() {
  ...
  for (const std::string& op : std::vector<std::string>{"create"}) {
    auto sk = is_control_skipped("entityOp", std::string("output_activate_store_module.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto output_activate_store_module_ref01_ent = client->output_activate_store_module();
