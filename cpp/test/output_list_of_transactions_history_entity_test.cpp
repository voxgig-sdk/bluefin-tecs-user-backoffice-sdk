...
static OutputListOfTransactionsHistorySetup output_list_of_transactions_history_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_LIST_OF_TRANSACTIONS_HISTORY_ENTID"));
...
static void output_list_of_transactions_history_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"output_list_of_transactions_history", vmap({
  ...
  auto se = strsdk->output_list_of_transactions_history();
  std::vector<Value> items = se->stream("list", Value::undef(), Value::undef());
  ASSERT_EQ((int)items.size(), 2, "stream yields both seeded items");

  auto plainsdk = BluefinTecsUserBackofficeSDK::testSDK(seed, Value::undef());
  auto pe = plainsdk->output_list_of_transactions_history();
...
static void output_list_of_transactions_history_entity_basic() {
  ...
  {
    Value output_list_of_transactions_history_ref01_data_result = output_list_of_transactions_history_ref01_ent->create(Struct::clone(output_list_of_transactions_history_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(output_list_of_transactions_history_entity_stream);
