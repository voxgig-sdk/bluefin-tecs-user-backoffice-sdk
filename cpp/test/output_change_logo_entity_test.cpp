...
static OutputChangeLogoSetup output_change_logo_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_CHANGE_LOGO_ENTID"));
...
static void output_change_logo_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"output_change_logo", vmap({
...
static void output_change_logo_entity_basic() {
  ...
  {
    Value output_change_logo_ref01_data_result = output_change_logo_ref01_ent->create(Struct::clone(output_change_logo_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(output_change_logo_entity_stream);
