// Generated direct-call tests for the output_get_logo entity (unit mode;
...
static OutputGetLogoDirectSetup output_get_logo_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  auto client = std::make_shared<BluefinTecsUserBackofficeSDK>(opts);

  OutputGetLogoDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void output_get_logo_direct_load() {
  auto setup = output_get_logo_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-output_get_logo", "unit");
...
int main() {
  T_RUN(output_get_logo_direct_load);
  return sdktest::summary("output_get_logo_direct_test");
}
