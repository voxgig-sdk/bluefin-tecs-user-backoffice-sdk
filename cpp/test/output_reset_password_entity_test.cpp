...
static OutputResetPasswordSetup output_reset_password_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_RESET_PASSWORD_ENTID"));
...
static void output_reset_password_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"output_reset_password", vmap({
...
static void output_reset_password_entity_basic() {
  ...
  {
    Value output_reset_password_ref01_data_result = output_reset_password_ref01_ent->create(Struct::clone(output_reset_password_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(output_reset_password_entity_stream);
