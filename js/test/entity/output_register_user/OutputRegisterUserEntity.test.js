...
describe('OutputRegisterUserEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        output_register_user: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.OutputRegisterUser().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.OutputRegisterUser().stream('list')) {
