...
describe('OutputListOfTransactionsHistoryEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        output_list_of_transactions_history: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.OutputListOfTransactionsHistory().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.OutputListOfTransactionsHistory().stream('list')) {
