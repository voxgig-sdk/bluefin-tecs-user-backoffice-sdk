...
func TestOutputDeactivateUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_DEACTIVATE_USER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		outputDeactivateUserRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "output_deactivate_user"}, setup.data), "output_deactivate_user_ref01"))

		outputDeactivateUserRef01DataResult, err := outputDeactivateUserRef01Ent.Create(outputDeactivateUserRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputDeactivateUserRef01Data = core.ToMapAny(outputDeactivateUserRef01DataResult)
...
func output_deactivate_userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_DEACTIVATE_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_DEACTIVATE_USER_ENTID"])
