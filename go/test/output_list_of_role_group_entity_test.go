...
func TestOutputListOfRoleGroupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_LIST_OF_ROLE_GROUP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		outputListOfRoleGroupRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "output_list_of_role_group"}, setup.data), "output_list_of_role_group_ref01"))

		outputListOfRoleGroupRef01DataResult, err := outputListOfRoleGroupRef01Ent.Create(outputListOfRoleGroupRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputListOfRoleGroupRef01Data = core.ToMapAny(outputListOfRoleGroupRef01DataResult)
...
func output_list_of_role_groupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_LIST_OF_ROLE_GROUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_LIST_OF_ROLE_GROUP_ENTID"])
