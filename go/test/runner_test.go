...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_EXPLAIN"); explain != "" {
