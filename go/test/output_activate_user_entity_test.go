...
func TestOutputActivateUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputActivateUserRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_activate_user"}), "output_activate_user_ref01"))

		outputActivateUserRef01DataResult, err := outputActivateUserRef01Ent.Create(outputActivateUserRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputActivateUserRef01Data = core.ToMapAny(entityData(outputActivateUserRef01DataResult))
...
func output_activate_userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_ACTIVATE_USER_ENTID"])
