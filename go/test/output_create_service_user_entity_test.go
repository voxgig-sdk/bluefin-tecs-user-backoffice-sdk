...
func TestOutputCreateServiceUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputCreateServiceUserRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_create_service_user"}), "output_create_service_user_ref01"))
		...
		outputCreateServiceUserRef01Data = core.ToMapAny(entityData(outputCreateServiceUserRef01DataResult))
...
func output_create_service_userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_SERVICE_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_SERVICE_USER_ENTID"])
