...
func TestOutputListOfAvailableRoleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputListOfAvailableRoleRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_list_of_available_role"}), "output_list_of_available_role_ref01"))
		...
		outputListOfAvailableRoleRef01Data = core.ToMapAny(entityData(outputListOfAvailableRoleRef01DataResult))
...
func output_list_of_available_roleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_LIST_OF_AVAILABLE_ROLE_ENTID"])
