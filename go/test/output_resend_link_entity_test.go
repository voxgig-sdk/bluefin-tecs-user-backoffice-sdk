...
func TestOutputResendLinkEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.OutputResendLink(nil)
		if ent == nil {
			t.Fatal("expected non-nil OutputResendLinkEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := output_resend_linkBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "output_resend_link." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		outputResendLinkRef01Data = core.ToMapAny(outputResendLinkRef01DataResult)
		if outputResendLinkRef01Data == nil {
...
func output_resend_linkBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "output_resend_link", "OutputResendLinkTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read output_resend_link test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse output_resend_link test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSUSERBACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID"])
