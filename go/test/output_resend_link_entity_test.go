...
func TestOutputResendLinkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		outputResendLinkRef01Data = core.ToMapAny(entityData(outputResendLinkRef01DataResult))
...
func output_resend_linkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_RESEND_LINK_ENTID")
