...
func TestOutputRegisterUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputRegisterUserRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_register_user"}), "output_register_user_ref01"))

		outputRegisterUserRef01DataResult, err := outputRegisterUserRef01Ent.Create(outputRegisterUserRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputRegisterUserRef01Data = core.ToMapAny(entityData(outputRegisterUserRef01DataResult))
...
func output_register_userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_REGISTER_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_REGISTER_USER_ENTID"])
