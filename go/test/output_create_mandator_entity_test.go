...
func TestOutputCreateMandatorEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputCreateMandatorRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_create_mandator"}), "output_create_mandator_ref01"))

		outputCreateMandatorRef01DataResult, err := outputCreateMandatorRef01Ent.Create(outputCreateMandatorRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputCreateMandatorRef01Data = core.ToMapAny(entityData(outputCreateMandatorRef01DataResult))
...
func output_create_mandatorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_MANDATOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_CREATE_MANDATOR_ENTID"])
