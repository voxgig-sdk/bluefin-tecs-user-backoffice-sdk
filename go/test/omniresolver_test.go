...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-tecs-user-backoffice-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinTecsUserBackofficeError); is {
				out := map[string]any{
					"name":    "BluefinTecsUserBackofficeError",
...
func omniCtx(arg any, client *sdk.BluefinTecsUserBackofficeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinTecsUserBackofficeSDK); is {
			client = live
