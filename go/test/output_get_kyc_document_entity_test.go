...
func TestOutputGetKycDocumentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputGetKycDocumentRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_get_kyc_document"}), "output_get_kyc_document_ref01"))

		outputGetKycDocumentRef01DataResult, err := outputGetKycDocumentRef01Ent.Create(outputGetKycDocumentRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputGetKycDocumentRef01Data = core.ToMapAny(entityData(outputGetKycDocumentRef01DataResult))
...
func output_get_kyc_documentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_GET_KYC_DOCUMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_USER_BACKOFFICE_TEST_OUTPUT_GET_KYC_DOCUMENT_ENTID"])
