...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*BluefinTecsUserBackofficeSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *BluefinTecsUserBackofficeError {
	return NewBluefinTecsUserBackofficeError(code, msg, ctx)
}
