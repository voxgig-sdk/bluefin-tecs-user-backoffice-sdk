...
//    only - a typed BluefinTecsUserBackofficeContext there would make every ctx
...
final class BluefinTecsUserBackofficeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BluefinTecsUserBackofficeContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BluefinTecsUserBackofficeSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BluefinTecsUserBackofficeResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BluefinTecsUserBackofficeError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BluefinTecsUserBackofficeResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BluefinTecsUserBackofficeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BluefinTecsUserBackofficeError(is_string($code) ? $code : '', $msg);
    }
