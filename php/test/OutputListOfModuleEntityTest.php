...
class OutputListOfModuleEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BluefinTecsUserBackofficeSDK::test(null, null);
        $ent = $testsdk->OutputListOfModule(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = output_list_of_module_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "output_list_of_module." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $output_list_of_module_ref01_ent = $client->OutputListOfModule(null);
...
function output_list_of_module_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/output_list_of_module/OutputListOfModuleTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["output_list_of_module01", "output_list_of_module02", "output_list_of_module03"] as $k) {
