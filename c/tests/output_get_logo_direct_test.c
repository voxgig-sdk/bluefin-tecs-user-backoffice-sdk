// Generated direct-call test for the output_get_logo entity (mirrors the
...
static voxgig_value* output_get_logo_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinTecsUserBackofficeSDK* output_get_logo_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(output_get_logo_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinTecsUserBackofficeSDK* sdk = output_get_logo_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_tecs_user_backoffice_output_get_logo_direct");
}
