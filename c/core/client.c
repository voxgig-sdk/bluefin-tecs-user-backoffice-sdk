// BluefinTecsUserBackofficeSDK client (generated — mirrors the rust Main fragment).
...
BluefinTecsUserBackofficeSDK* bluefintecsuserbackoffice_sdk_new(voxgig_value* options) {
  BluefinTecsUserBackofficeSDK* sdk = (BluefinTecsUserBackofficeSDK*)calloc(1, sizeof(BluefinTecsUserBackofficeSDK));
...
static bool sdk_op_allowed(BluefinTecsUserBackofficeSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(BluefinTecsUserBackofficeSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  BluefinTecsUserBackofficeSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(BluefinTecsUserBackofficeSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "BluefinTecsUserBackofficeSDK: graphql: %s", m);
...
BluefinTecsUserBackofficeSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinTecsUserBackofficeSDK* sdk = bluefintecsuserbackoffice_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
